...
- 🚨 **크롤 데이터의 `price`가 29CM 실제 표시가와 다르다** (2026-08-05 발견). 파서가 `itemInfo.sellPrice`를
  담는데, 상품 페이지의 최종 판매가는 그보다 낮았다 — 코즈넉 크롤 8,900 vs 실제 6,520 / 르 17,550 vs 13,430 /
  라미네즈 21,750 vs 16,650 / 몽돌 28,050 vs 21,720. `discount_rate`(saleRate)는 **실제 표시가 기준**이라
  `original_price`→`price` 로 검산하면 어긋난다(9,900→8,900은 10%인데 필드값 34%). 무신사는 일치.
  → **카드에 가격을 넣기 전 반드시 상품 페이지에서 실제 표시가를 확인할 것.** 파이프라인 자동 카피는
  이 값을 그대로 쓰므로 과거 게시물에도 틀린 가격이 나갔을 수 있다.
