...
# 캡션(인스타 게시글 본문) 톤 — 깔끔·절제 톤
- 첫 줄은 제목형으로 짧게 시작합니다.
- 이어서 담백한 2~3문장으로 설명합니다. 근거는 한 문장에 녹여 쓰고, 짧게 끊어 나열하지 않습니다.
- 독자의 경험을 소환하는 질문을 하나 포함하되, "댓글로 알려주세요" 같은 상투적 마무리는 쓰지 않습니다.
- **숫자 스펙을 캡션에서 브리핑하지 않습니다.** 평점·후기 수·가격 비교는 카드가 이미 보여주는
  정보이므로 캡션에서 반복하지 않습니다. 사실을 대칭 구조로 정리해 나열하는 문장이 가장 AI 같습니다.
- 덜 AI처럼 보이려고 오글거리는 구어체를 섞지 않습니다 (절제 톤은 짧을수록 힘이 붙습니다).
- 이모지는 본문 전체에 3~5개를 주제와 어울리게 문장 끝에 배치합니다.
- 해시태그는 절대 사용하지 않습니다 (한 개도 포함 금지).
- 말줄임표(...·…)와 과도한 감탄사를 사용하지 않습니다.
- **브랜드 인스타 계정 핸들(@...)을 지어내지 않습니다.** 캡션 하단의 브랜드 계정 목록은
  게시 전 사람이 3회 독립 검증해 붙이므로, 여기서는 본문만 씁니다.
...
_RESERVATION_RE = re.compile(r"긴\s*(하|합|해|했|한)|그럭저럭|나쁘지\s*않|무난하|그래도|그치만|그렇지만")
...
_STRONG_WORDS = ("너무", "진짜", "완전", "강추", "추천", "재구매", "최고", "마음에", "만족", "확실히", "강력")
...
_MOOD = {
    "봄": {"kicker": "SPRING MOOD", "cover_title": "살랑이는 계절엔<br><em>가볍게</em>",
           ...
           "emoji": "🌿",
           "cap_lead": "완연한 봄, 가볍게 걸치기 좋은 것들로 골랐습니다 ☀️",
           "cap_q": "요즘 가장 먼저 손이 가는 한 벌은 무엇인가요 💬"},
...
def fallback_copy(products: list[dict], topic: str, month: int | None = None) -> dict:
    ...
    # 캡션 규칙(KEYWORD-POLICY): 이모지 3~5개 · 해시태그 금지 · 숫자 스펙 브리핑 금지
    # ("후기와 평점을 함께 살펴봤습니다" 류의 나열·브리핑 문장은 카드가 이미 보여주는 정보라 뺀다).
    # 이 폴백 캡션도 게시 전에는 에이전트가 반드시 재작성한다 — 규칙 위반 상태로 두지 않기 위한 기본값이다.
    caption = (
        f"{mood['cover_sub']} {mood['emoji']}\n\n"
        f"{mood['cap_lead']}\n\n"
        f"{mood['cap_q']}"
    )
