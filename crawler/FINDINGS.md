...
`categoryFacetInputs:[{"largeId":268100100,"middleId":<중분류코드>}]` 가 실린다.
- 상의 268103100 / 바지 268106100 / 원피스 268104100 / 스커트 268107100
- 가방(269100100)·여성슈즈(270100100)는 대분류(large)만으로 필터.
- 크롤러는 상의·바지·원피스/스커트(원피스+스커트 교차 병합)·가방·신발 5종 + 전체(best) 수집.

## 29CM 카테고리 필터 (2026-07-21 추가 실측)

- 베스트 페이지의 카테고리 칩 클릭 시 POST 바디에 `facets.categoryFacetInputs: [{"largeId": <대분류코드>}]` 가 추가됨 — **카테고리 필터 가능 확인** (Phase 0 의 "필터 불가" 결론을 갱신).
- 브라우저 없이 requests.post 로도 동일하게 동작 (여성가방 269100100 으로 100건 실측).
- 확보한 대분류 코드: 여성의류 268100100 / 여성가방 269100100 / 여성슈즈 270100100 / 여성액세서리 271100100 / 여성주얼리 305100100 / 여성모자 310100100.
...
## 무신사 상품 상세 이미지 (2026-08-11 확인)

- 상품 페이지 `https://www.musinsa.com/products/{id}` 는 **JS 렌더**라 HTML 에 이미지 URL 이 없다
  (msscdn 링크 0건). 카드뉴스 표지용 "다른 컷"을 구하려면 아래 API 를 쓴다.
- 핵심 URL: `GET https://goods-detail.musinsa.com/api2/goods/{goodsNo}` (인증 불필요, 200)
- 응답 구조: `{meta, data, error}` — 이미지는 `data` 안에 있다.

  | 필드 | 내용 |
  |---|---|
  | `data.thumbnailImageUrl` | 목록/랭킹에 쓰는 대표 썸네일 (`/images/goods_img/...`) |
  | `data.goodsImages[].imageUrl` | **추가 컷 배열** (`/images/prd_img/{날짜}/{id}/detail_{id}_{ts}_500.jpg`) |
  | `data.goodsMaterial` | 소재 정보 (랭킹 API 에는 없음) |
  | `data.goodsPrice` / `data.price` | 가격 상세 (크롤 랭킹값이 정가인 경우가 있어 대조용으로 유용) |

- 경로는 상대경로이므로 앞에 `https://image.msscdn.net` 를 붙인다.
- 실측 예: `6331313`(크랭크 시어 컬러 버튼 가디건) → `goodsImages` 4장
  (전신 착용컷 / 상반신 / 디테일 / 누끼). 표지에 쓸 만한 건 얼굴 나온 착용컷뿐이었다.
- ⚠️ **랭킹 API 의 `price` 가 실제 표시가와 다를 수 있다.** 같은 상품에서 랭킹은 59,000원
  (정가), 상세는 50,450원(14% 할인가)이었다. 카드에 넣을 가격은 상세에서 재확인할 것.
